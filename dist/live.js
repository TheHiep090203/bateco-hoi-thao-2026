...
function renderLive(){renderDraws();document.querySelector('#huy-chuong tbody').innerHTML=liveData.standings.map(a=>`<tr data-team="${a.id}" data-rank="${a.rank}"><th><span class="team-line team-${a.id-1}"></span>LIÊN MINH ${safe(a.name)}</th><td class="lead-metric">${a.gold}</td><td>${a.silver}</td><td>${a.bronze}</td><td>${a.gold+a.silver+a.bronze}</td><td><span class="rank" aria-label="Hạng ${a.rank}">${a.rank}</span>${a.tied?'<small class="tie">Đồng hạng</small>':''}</td></tr>`).join('');document.querySelector('#huy-chuong .section-head>span').textContent=liveData.standings.some(a=>a.gold+a.silver+a.bronze)?'Cập nhật theo số huy chương đã nhập':'Chưa có huy chương được cập nhật';document.querySelector('#result-panel').innerHTML=resultTable(selectedSport);if(dialog.open&&lastButton)renderDialogResults(+lastButton.dataset.sport)}
...
async function refreshAdminSession(){let next=adminSession;
 ...
 if(next!==adminSession){adminSession=next;if(liveData)renderLive()}}
sync().then(refreshAdminSession);setInterval(()=>{if(!document.hidden)sync()},5000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)sync()});window.addEventListener('online',sync);
